...
```
$CLAUDE_PROJECT_DIR/.claude/error-cache/[session_id]/
```

**Note**: Renamed from `tsc-cache` to `error-cache` to support multiple languages (Python, Java, TypeScript, Bash).

### Cache Files

- `last-errors.txt` - Last compilation/syntax errors
- `affected-repos.txt` - List of affected repositories
- `check-commands.txt` - Language-specific check commands (Python, Java, TypeScript, Bash)
- `build-commands.txt` - Build commands for each repo
- `edited-files.log` - Log of edited files

### Manual Cache Cleanup

```bash
# Remove all cached data
rm -rf $CLAUDE_PROJECT_DIR/.claude/error-cache/*

# Remove specific session
rm -rf $CLAUDE_PROJECT_DIR/.claude/error-cache/[session-id]
```
